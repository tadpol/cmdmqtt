...
command :config do |c|
  c.syntax = %{mqtt config [options] <key> [<new value>]}
  ...
  c.example %{See what the current combined config is}, 'mqtt config --dump'
  c.example %{Query a value}, 'mqtt config jira.project'
  c.example %{Set a new value; writing to the project config file}, 'mqtt config jira.project XXXXXXXX'
  c.example %{Set a new value; writing to the user config file}, 'mqtt config --user user.name my@email.address'
  c.example %{Unset a value in a configfile. (lower scopes will become visible if set)},
    'mqtt config diff.cmd --unset'
  ...
  c.action do |args, options|
    ...
    if options.dump then
      ...
      scopes = CmdMqtt::Config::CFG_SCOPES if scopes.empty?
